...
> 考察期需要 bot 有群管理员权限（仅用于 `set_group_kick`）。

**考察标准增强（可选）**

- `min_messages`（默认 0）：转正消息数阈值。设为 N 后，进群后累计发言达到 N 条即转正，不再只凭"是否发过言"；0 = 不启用，保持原行为。
- `llm_judge`（默认 false）：考察期满仍未能转正时，用 LLM 判定该号是否为正常号/真人。**明确判定为异常（广告号、机器人号、僵尸号、水军号等）才移出，其余一律保守保留**，避免误踢真人；关闭则到期直接移出（原行为）。

> 注意：开启 `llm_judge` 后，考察期到期时会调用一次 LLM（走 `[probation].llm` 槽位）。它只输出"正常 / 异常"作为参考，真正移出仍由移出时机的确定性判断决定，机器人不会凭空自主决定动谁。
